...
- **Instance selection:** version stable capability requirements and replacement filters, not live instance identity or price. Query the current inventory with `brev ls` and select replacements from a fresh, reviewed `brev search`; see [`instance-selection.md`](instance-selection.md).
